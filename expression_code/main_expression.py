...
class Model(QThread):
    ...
    def run(self):
        self.image, self.dictionary = self.apply_expression(self.image_path, expression=self.expression)
        self.updated.emit()
    ...
    def apply_expression(self, original_image_path, path_log='log/', expression='angry'):
        ...
        projShape = np.transpose(
            _3DMM_obj.getProjectedVertex(np.transpose(shape_neutral_model), pos_est["S"], pos_est["R"], pos_est["T"]))
        ...
        [frontalView, colors, mod3d] = _graph_tools_obj.renderFaceLossLess(shape_expressional_model, projShape, image,
                                                                           pos_est["S"], pos_est["R"], pos_est["T"], 1,
                                                                           pos_est["visIdx"])
    ...
    def apply_expression_modelPreloaded(self, dict_, path_log='log/', expression='angry'):
        # define RP values
        RP_obj = RP()
        # Fitting and pose estimation are within the 3DMM obj
        _3DMM_obj = _3DMM()
        ...
        shape_expressional_model = np.transpose(
            _3DMM_obj.deform_3D_shape_fast(np.transpose(dict_['shape_neutral']), dict_['components'], vect))

        # create texture for expressional model
        projShape = np.transpose(
            _3DMM_obj.getProjectedVertex(np.transpose(shape_expressional_model), dict_['pos_est_S'], dict_['pos_est_R'],
                                         dict_['pos_est_T']))
        # texture_expressional_model = (_graph_tools_obj.getRGBtexture(projShape, image))*255
